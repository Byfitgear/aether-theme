...
class Aether_Theme_Updater {
    ...
    public function __construct() {
        ...
        $this->update_path = 'https://www.aetherwp.dev/api/wp-updates/themes/check';
    ...
    private function get_remote_version() {
        $cache_key = 'aether_theme_update_' . $this->theme_slug;
        ...
        // 在开发环境下添加 dev 参数
        if (defined('WP_DEBUG') && WP_DEBUG && defined('AETHER_DEV_MODE') && AETHER_DEV_MODE) {
            $url .= '&dev=1';
        }
        ...
        $args = array(
            ...
            'sslverify' => true,
            'headers' => array(
                'User-Agent' => 'aether-theme-updater/' . $this->version . '; ' . get_bloginfo('url'),
            ),
        );
        ...
        if ($response_code === 200) {
            ...
            if (isset($data['update_available'])) {
                // 成功结果缓存更久；开发模式下缩短
                $success_ttl = (defined('AETHER_DEV_MODE') && AETHER_DEV_MODE) ? 5 * MINUTE_IN_SECONDS : 12 * HOUR_IN_SECONDS;
    ...
    private function get_remote_info() {
        $cache_key = 'aether_theme_info_' . $this->theme_slug;
        $cached = get_site_transient($cache_key);
        if ($cached) {
            return $cached;
        }
        ...
        $request = wp_remote_get('https://www.aetherwp.dev/api/wp-updates/themes/info/' . $this->theme_slug, $args);
    ...
    private function get_changelog() {
        $cache_key = 'aether_theme_changelog_' . $this->theme_slug;
        $cached = get_site_transient($cache_key);
        if ($cached !== false) {
            return $cached;
        }
        ...
        $request = wp_remote_get('https://www.aetherwp.dev/api/wp-updates/themes/changelog/' . $this->theme_slug, $args);
